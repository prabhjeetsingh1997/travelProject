...
<div aria-hidden="true" aria-labelledby="myModalLabel" role="dialog" tabindex="-1" id="sendToCl" class="modal fade">
	<div class="modal-dialog mail-modal">
	   <form name="tour_mail" id="tour_mail" method="post" action="">
			...
			<input type="hidden" name="tourSub" id="tourSub" value="Lid Travel Quotation" />
...
$(function() {
	...
	$(document).on('change','.hoteltypes',function(){
		...
		$.ajax({
			type:"POST",
			url:"<?php echo base_url(ADMIN_URL).'/getHotelRoomsForPackage'?>",
			data:data,
			beforeSend:function(){
	...
	$(document).on('change','.selectVehicle',function(){
		...
		$.ajax({
			type:"POST",
			url:"<?php echo base_url(ADMIN_URL).'/getVehicleVendorName'?>",
			data:data,
			beforeSend:function(){
	...
	$("#calculate_prices").click(function(){
		    ...
		    $.ajax({
			type:"POST",
			url:"<?php echo base_url(ADMIN_URL).'/getpakageHotelPrice'?>",
	...
	$("#sel_vehicle").on('change', function(){
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url();?>User/getselvehicleperunitcost/"+veh_id,
			data: {id:veh_id},
			//cache: false,
			beforeSend:function() {
